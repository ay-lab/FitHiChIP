...
ConfigFileDir="$(cd $(dirname $ConfigFile); pwd)"
BINDARG=("-v$ConfigFileDir:$ConfigFileDir")
#BINDARG+=("-v$TMPDIR:$TMPDIR")
...
if [[ ! -z $InpValidPairsFile ]]; then
	...
	BINDARG+=("-v$(dirname $InpValidPairsFile):$(dirname $InpValidPairsFile)")
fi
...
if [[ ! -z $InpBinIntervalFile ]]; then
	...
	BINDARG+=("-v$(dirname $InpBinIntervalFile):$(dirname $InpBinIntervalFile)")
fi
...
if [[ ! -z $InpMatrixFile ]]; then
	...
	BINDARG+=("-v$(dirname $InpMatrixFile):$(dirname $InpMatrixFile)")
fi
...
if [[ ! -z $PeakFILE ]]; then
	...
	BINDARG+=("-v$(dirname $PeakFILE):$(dirname $PeakFILE)")
fi
...
if [[ ! -z $RefFastaFile ]]; then
	...
	BINDARG+=("-v$(dirname $RefFastaFile):$(dirname $RefFastaFile)")
fi
...
if [[ ! -z $REFragFile ]]; then
	...
	BINDARG+=("-v$(dirname $REFragFile):$(dirname $REFragFile)")
fi
...
if [[ ! -z $ChrSizeFile ]]; then
	...
	BINDARG+=("-v$(dirname $ChrSizeFile):$(dirname $ChrSizeFile)")
fi
...
if [[ ! -z $MappabilityFile ]]; then
	...
	BINDARG+=("-v$(dirname $MappabilityFile):$(dirname $MappabilityFile)")
fi
...
if [[ ! -z $OutDir ]]; then
	...
	BINDARG+=("-v$OutDir:$OutDir")
fi
DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"

#echo ${BINDARG[@]}
docker run "${BINDARG[@]}" --entrypoint /bin/bash aylab/fithichip /FitHiChIP/FitHiChIP_HiCPro.sh -C "$(cd $(dirname $ConfigFile); pwd)/$(basename $ConfigFile)" -s
#bash "$DIR/FitHiChIP_HiCPro.sh" -C "$ConfigFile" -s
